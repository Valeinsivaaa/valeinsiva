...
setInterval(async () => {
    try {
        ...
        if(currentGame) db.lastGame = { ...currentGame };
        io.emit("presence", { ...data, lastSpotify: db.lastSpotify, lastGame: db.lastGame, isGameActive: !!currentGame });
    } catch (e) {}
}, 2000);
...
app.get("/api/like", async (req, res) => { db.likes++; await syncWithGithub(true); res.json({ success: true }); });
app.get("/api/view", async (req, res) => { db.views++; await syncWithGithub(true); res.json({ success: true }); });
...
<head>
    ...
    <style>
        ...
        .act-card { border-radius: 20px; padding: 12px; border: 1px solid rgba(255,255,255,0.05); text-align: left; display: flex; align-items: center; gap: 12px; position: relative; }
        .act-img { width: 52px; height: 52px; border-radius: 14px; object-fit: cover; }
        ...
        .act-detail { font-size: 10px; opacity: 0.5; }

        /* Spotify Bar Fix */
        .progress-bar { height: 4px; background: rgba(255,255,255,0.08); border-radius: 10px; margin-top: 8px; width: 100%; position: relative; }
        .progress-fill { height: 100%; background: #1db954; width: 0%; border-radius: 10px; transition: width 1s linear; }
        ...
        .nav-btn { width:46px; height:46px; background:var(--card); border-radius:50%; display:flex; align-items:center; justify-content:center; border:1px solid rgba(255,255,255,0.1); cursor:pointer; color:#777; transition: 0.3s; }
...
<body>
    ...
    <div class="wrapper">
        <div class="main-card">
            ...
            <div style="padding:0 20px 20px; text-align:center;">
                ...
                <div style="display:flex; justify-content:center; gap:30px; margin-top:15px; padding-top:15px; border-top:1px solid rgba(255,255,255,0.04);">
                    <a href="https://discord.com/users/${DISCORD_ID}" target="_blank" style="color:#7289da; text-decoration:none;"><i class="fa-brands fa-discord fa-xl"></i></a>
                    <a href="${INSTAGRAM_LINK}" target="_blank" style="color:#E1306C; text-decoration:none;"><i class="fa-brands fa-instagram fa-xl"></i></a>
                    <a href="${BOT_PANEL_LINK}" target="_blank" style="color:#00d2ff; text-decoration:none;"><i class="fa-solid fa-layer-group fa-xl"></i></a>
                </div>
            </div>
        </div>
        
        <div id="msg-container" style="background:var(--card); border-radius:30px; padding:20px; border:1px solid rgba(255,255,255,0.06);">
             <div id="msg-feed"></div>
             <div id="msg-form-area" style="margin-top:15px;">
                <input id="in-user" placeholder="Sen kimsin?" style="width:100%; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:10px; color:white; margin-bottom:8px; outline:none;">
                <textarea id="in-text" placeholder="Mesajın..." style="width:100%; height:60px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:10px; color:white; resize:none; margin-bottom:10px; outline:none;"></textarea>
    ...
    <script>
        ...
        socket.on("presence", data => {
            ...
            if(game) {
                const icon = game.application_id ? \`https://cdn.discordapp.com/app-icons/\${game.application_id}/\${game.assets?.large_image}.png\` : "https://i.ibb.co/9vX78f7/ps5.png";
                html += \`
                <div class="act-card" style="background:rgba(114,137,218,0.05);">
                    <img src="\${icon}" class="act-img" onerror="this.src='https://i.ibb.co/9vX78f7/ps5.png'">
                    <div class="act-info">
                        <div class="act-label" style="color:var(--accent);">\${data.isGameActive ? 'ŞU AN OYUNDA' : 'SON OYNANAN'}</div>
                        <div class="act-name">\${game.name}</div>
                        <div class="act-detail" id="game-timer">\${data.isGameActive ? '00:00:00' : 'Çevrimdışı'}</div>
                    </div>
                    <i class="fa-brands fa-playstation" style="position:absolute; right:15px; top:15px; opacity:0.2;"></i>
                </div>\`;
            }
            ...
            if(spot) {
                html += \`
                <div class="act-card" style="background:rgba(30,215,96,0.05); flex-direction:column; align-items:flex-start;">
                    <div style="display:flex; gap:12px; align-items:center; width:100%;">
                        <img src="\${spot.album_art_url}" class="act-img">
                        <div class="act-info">
                            <div class="act-label" style="color:#1db954;">\${data.spotify ? 'DINLIYOR' : 'SON DINLENEN'}</div>
                            <div class="act-name">\${spot.song}</div>
                            <div class="act-detail">\${spot.artist}</div>
                        </div>
                    </div>
                    \${data.spotify ? '<div class="progress-bar"><div id="s-fill" class="progress-fill"></div></div><div style="display:flex; justify-content:space-between; width:100%; font-size:9px; opacity:0.4; margin-top:4px;"><span id="s-cur">00:00</span><span id="s-tot">00:00</span></div>' : ''}
                </div>\`;
            }
            list.innerHTML = html;
        });

        // Saniyelik Akış Motoru
        setInterval(() => {
            if(currentData?.isGameActive && currentData.lastGame.timestamps?.start) {
                ...
                if(timerEl) timerEl.innerText = formatFullTime(elapsed) + " süredir oynuyor";
            }
            if(currentData?.spotify) {
                ...
                if(tot) {
                    ...
                    tot.innerText = Math.floor(s/60).toString().padStart(2,'0') + ":" + (s%60).toString().padStart(2,'0');
                }
            }
        }, 1000);
        ...
        function sendMsg() {
            ...
            if(u && t) { socket.emit('send_msg', {user:u, text:t}); localStorage.setItem('v_msg_sent', 'true'); checkMsgStatus(); }
        }

        function checkMsgStatus() {
            if(localStorage.getItem('v_msg_sent')) {
                document.getElementById('msg-form-area').innerHTML = '<div style="text-align:center; color:#ff4757; font-size:11px; font-weight:800;">Mesaj hakkın bitti!</div>';
            }
        }
        ...
        function renderMsgs(m) {
            document.getElementById("msg-feed").innerHTML = m.map(x => \`
                <div style="background:rgba(255,255,255,0.02); padding:10px; border-radius:15px; margin-bottom:8px; border-left:3px solid var(--accent);">
                    <div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:3px;">
                        <b style="color:var(--accent);">\${x.user}</b>
                        <span style="opacity:0.3;">\${Math.floor((Date.now()-x.time)/1000/60)} dk önce</span>
                    </div>
        ...
        document.getElementById("btn-like").onclick = function() {
            ...
            fetch('/api/like').then(() => { this.classList.add('liked'); localStorage.setItem('v_liked', 'true'); });
        };
